...
- General `Proj` infrastructure for the scheme-theoretic projective line.  This
  ...
  ideals over a domain, and the resulting `Proj` points `[0:1]` and `[1:0]`
  in `HilbertTest.SourceStack.SchemeProjectiveLine`; still missing is the
  `(X0-X1)` point and marked-point identification.
