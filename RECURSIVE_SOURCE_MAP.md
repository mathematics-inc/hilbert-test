...
| Algebraic point Galois conjugacy | Mathlib field theory; Stacks Project field extensions; standard primitive-element theorem | Checked primitive-element, finite adjoin, separability, normal/Galois, splitting, minimal-polynomial conjugacy, derivative-root degree-drop, and normal-tower restriction wrappers in `SourceStack.FieldTheory`; checked finite polynomial images and the named derivative-root replacement-set API, including separation and derivative-nonvanishing away from the replacement set, in `SourceStack.PolynomialMaps`; still needs `P^1(Qbar)` point/model bridge |
