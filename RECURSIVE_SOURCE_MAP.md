...
- General `Proj` infrastructure for the scheme-theoretic projective line.  This
  ...
  finite marked-point triple of cardinality three.  The common-label bridge
  between this scheme triple and the linear projective-line branch triple is
  checked in `HilbertTest.SourceStack.MarkedProjectiveLine`, and the abstract
  Belyi-cover finite-branch-set interface has been specialized to this scheme
  triple in `HilbertTest.SourceStack.SchemeMarkedBelyi`.  Still missing is the
  morphism/rational-function API using these marked points as branch targets.
