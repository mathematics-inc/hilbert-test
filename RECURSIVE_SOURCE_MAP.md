...
- The vector-space lemma that a finite union of proper subspaces does not cover
  ...
  actual two standard affine `P1` charts via `Proj.awayι`, packages the result
  as a `ProjectiveLineSectionPair`, and proves that finite marked Belyi maps
  built from such pairs feed into the section-controlled finite marked bridge.
  The
