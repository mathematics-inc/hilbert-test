...
| Algebraic point Galois conjugacy | Mathlib field theory; Stacks Project field extensions; standard primitive-element theorem | Checked primitive-element, finite adjoin, separability, normal/Galois, splitting, minimal-polynomial conjugacy, derivative-root degree-drop, and normal-tower restriction wrappers in `SourceStack.FieldTheory`; still needs `P^1(Qbar)` point/model bridge |
