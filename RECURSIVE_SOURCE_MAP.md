...
| Rational maps/function fields | Mathlib `AlgebraicGeometry.RationalMap`; Mathlib `AlgebraicGeometry.FunctionField`; Stacks Project rational maps and function fields | Checked dense-domain and partial-map representative wrappers in `SourceStack.RationalMaps`; checked function-field injection/fraction-field/generic-point wrappers in `SourceStack.FunctionFields`; still needs curve-specific divisors/Riemann-Roch |
...
| Local compactness in Corollary 3.2 | Lorscheid; Serre/Cassels local fields; Stacks properness | Proper local-field point spaces are compact in the strong topology |
