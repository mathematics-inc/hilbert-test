...
- Define Belyi maps as finite morphisms `C -> P^1` whose branch locus is
  ...
  finite set is now checked, while the specialized branch-locus/morphism API is
  still missing.
...
- General `Proj` infrastructure for the scheme-theoretic projective line.  This
  ...
  target and for maps whose target type is the scheme carrier `P1 K`.  Still
  missing is the morphism/rational-function API using these marked points as
  branch targets.
