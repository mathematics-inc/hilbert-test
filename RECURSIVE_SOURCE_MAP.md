...
- Define Belyi maps as finite morphisms `C -> P^1` whose branch locus is
  ...
  branch-complement target, its source-open membership criterion, and a finite
  Belyi-map refinement with finite-restriction consequences.  The specialized
...
- The vector-space lemma that a finite union of proper subspaces does not cover
  ...
  from the two sections `s0,s1`, packages the missing two-section morphism to
  `P1 K`, and proves that finite marked Belyi maps built from such pairs feed
  ...
  now constructs the bad target set `aux(S) ∪ badValues`, provides the
  corresponding `P1ReductionStep` constructor, checks the composition-through-
  `P1` set control, and turns a family of reduction steps into
  `FiniteMarkedBelyiExistence`.  The inclusion-exclusion estimates remain
  separate.
