...
- `HilbertTest.SourceStack.CurveRiemannRoch`: curve-facing wrapper around the
  checked linear algebra, packaging section evaluations, finite-set vanishing as
...
- `HilbertTest.SourceStack.ProjectiveSectionMaps`: projective-section source
  ...
  nonzero on `T`, checks the scheme-gluing bridge from compatible local chart
  morphisms on an open cover to a global morphism `C ⟶ P1 K`, descends the
  local zero-section criteria to that global map, specializes the local maps to
...
- `HilbertTest.SourceStack.SchemeProjectiveLine`: the concrete scheme
  ...
  triple of cardinality three, re-exported as a finset and finite set on the
  scheme carrier `P1 K`, with their expected membership/nonmembership in the
...
- `HilbertTest.SourceStack.SchemeMarkedBelyi`: specialization of the abstract
  ...
  marked-avoidance forms for scheme-carrier maps and honest scheme morphisms,
  both for the raw projective-spectrum target and for maps whose target type is
  the scheme carrier `P1 K`, plus
...
- `HilbertTest.SourceStack.PolynomialValueSurjectivity`: algebraically closed
  ...
  outside the finite forbidden set; nonzero derivative implies the needed
  positive-degree hypothesis after coefficient extension, so it produces an
...
- `HilbertTest.SourceStack.SchemeAffineLinePoints`: concrete construction of
  ...
  marked-triple membership exactly for `r = 0` or `r = 1`, plus the scheme
  four-point set `{0,r,1,infinity}` and the concrete `LinearSchemePointBridge`
  from the linear projective-line model to the scheme carrier.
...
- `HilbertTest.SourceStack.Schemes`: finite/smooth/proper/etale morphism
  ...
  spanning the unit ideal, its two-section Bezout specialization, and the
  top-section restriction bridge for local representatives.
...
- `HilbertTest.SourceStack.SchemeBelyi`: scheme-level abstract Definition 1.1
  ...
  source Belyi open and the source-open carrier itself with avoidance of the
  checked marked triple, including T1-specialized forms that require no explicit
  branch-open hypothesis.  The finite
...
- nonconstant rational function `X -> P^1` induces a finite morphism.
  ...
  family of sections, the two-section Bezout cover specialization, and the
  top-section restriction bridge for local representatives.  The
...
- Lemma 2.4: Galois-stable induction on degree of algebraic points using minimal
  ...
  over an algebraically closed target field a polynomial with nonzero derivative
  realizes such an outside value as `p(beta)`; `SourceStack.P1PolynomialSeparation`
