...
/-- The two-element family indexed by `Fin 2`. -/
def twoElementFamily {R : Type*} (x y : R) : Fin 2 → R
  | 0 => x
  | 1 => y

@[simp]
theorem twoElementFamily_zero {R : Type*} (x y : R) :
    twoElementFamily x y 0 = x := rfl

@[simp]
theorem twoElementFamily_one {R : Type*} (x y : R) :
    twoElementFamily x y 1 = y := rfl

/-- A Bezout equation for two elements proves that the ideal generated by the
corresponding two-element family is the unit ideal. -/
theorem ideal_span_range_twoElementFamily_eq_top_of_linear_combination
    {R : Type*} [CommSemiring R] (x y a b : R)
    (h : a * x + b * y = 1) :
    Ideal.span (Set.range (twoElementFamily x y)) = ⊤ := by
  rw [Ideal.eq_top_iff_one]
  rw [← h]
  have hx : x ∈ Ideal.span (Set.range (twoElementFamily x y)) :=
    Ideal.subset_span ⟨0, by simp [twoElementFamily]⟩
  have hy : y ∈ Ideal.span (Set.range (twoElementFamily x y)) :=
    Ideal.subset_span ⟨1, by simp [twoElementFamily]⟩
  exact Ideal.add_mem _
    (Ideal.mul_mem_left _ a hx)
    (Ideal.mul_mem_left _ b hy)

/-- The two-basic-open cover attached to a pair of global sections satisfying a
Bezout equation. -/
def twoSectionBasicOpenCoverOfLinearCombination
    (X : Scheme.{u}) (s0 s1 a b : Γ(X, ⊤))
    (h : a * s0 + b * s1 = 1) :
    X.OpenCover :=
  basicOpenCoverOfSpanEqTop X (twoElementFamily s0 s1)
    (ideal_span_range_twoElementFamily_eq_top_of_linear_combination s0 s1 a b h)
