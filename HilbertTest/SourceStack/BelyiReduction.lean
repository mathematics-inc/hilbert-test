...
/-- Combined map-level, openness, and open-control version of the one-step
reduction: the selected composed finite Belyi map has the prescribed marked
controls, its source Belyi open is open, and this open contains `T` while
avoiding `S`. -/
theorem exists_composedMap_controls_and_isOpen_belyiOpen_controls_of_p1MapExistence_auxEtale
    {Φ : Type z}
    (F : FiniteMarkedBelyiExistence K Φ (P1 K))
    (hS : S.Finite)
    (badValues : Set (P1 K)) (hbad : badValues.Finite)
        ...
        composed.hom = aux ≫ (F.map φ).hom ∧
          ((∀ x ∈ S, composed.hom.base x ∈ markedSchemePointSet K) ∧
            ...
            IsOpen (composed.toBelyiMap.belyiOpen : Set C) ∧
              T ⊆ (composed.toBelyiMap.belyiOpen : Set C) ∧
                (composed.toBelyiMap.belyiOpen : Set C) ⊆ Sᶜ := by
  rcases exists_composedMap_controls_and_belyiOpen_controls_of_p1MapExistence_auxEtale
      (K := K) (C := C) (S := S) (T := T) F hS badValues hbad aux
      (targetPoint := targetPoint) himage htargetBad maps_T_to_target hAuxEtale with
    ⟨φ, composed, hhom, hcontrols, hTopen, hopenS⟩
  exact ⟨φ, composed, hhom, hcontrols, composed.toBelyiMap.belyiOpen.2, hTopen, hopenS⟩
...
/-- Combined map-level, openness, and open-control version of
`exists_of_auxiliaryData`: fixed-pair auxiliary source material directly
produces a composed finite Belyi map with marked controls, an explicitly open
source Belyi open, and Belyi-open controls. -/
theorem exists_composedMap_controls_and_isOpen_belyiOpen_controls_of_auxiliaryData
    {Φ : Type z}
    (F : FiniteMarkedBelyiExistence K Φ (P1 K))
    (hS : S.Finite)
    (D : P1ReductionAuxiliaryData K C F S T) :
  ...
  letI : IsFinite D.aux := D.aux_finite
  letI : IsDominant D.aux := D.aux_dominant
  exact exists_composedMap_controls_and_isOpen_belyiOpen_controls_of_p1MapExistence_auxEtale
    (K := K) (C := C) (S := S) (T := T) F hS
    D.badValues D.badValues_finite D.aux
    (targetPoint := D.targetPoint)
    D.image_avoids_target D.target_not_bad D.maps_T_to_target
...
/-- Direct same-map finite disjoint-set conclusion from a global `P1`-reduction
family: the selected finite Belyi map satisfies the marked controls and its
Belyi open contains `T` and avoids `S`. -/
...
theorem exists_map_belyiOpen_inside_open_of_finite_complement
  ...
  rcases
      FiniteMarkedBelyiExistence.exists_map_belyiOpen_inside_open_of_finite_complement
        K (ReductionIndex C) E.toFiniteMarkedBelyiExistence hU hUcompl hxU with
    ⟨i, hopen, hxopen, hsub⟩
  exact
    ⟨i,
      (by simpa [toFiniteMarkedBelyiExistence_map_apply] using hopen),
